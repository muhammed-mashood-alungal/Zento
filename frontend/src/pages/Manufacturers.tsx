...
const Manufacturers: React.FC = () => {
  ...
  const { control, handleSubmit, reset, formState: { errors } } = useForm<ManufacturerFormData>({
    defaultValues: {
      name: '',
      description: '',
      status: 'active',
  ...
  const handleEdit = (item: CardItem) => {
    const manufacturer = manufacturers.find(m => m.id.toString() === item.id.toString());
    if (manufacturer) {
  ...
  const handleDelete = (item: CardItem) => {
    setManufacturers(manufacturers.filter(m => m.id.toString() !== item.id.toString()));
  };
